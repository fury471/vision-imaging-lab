...
from image_transforms import scale_contrast
...
low_contrast_image = scale_contrast(gray_image, alpha=0.7)
high_contrast_image = scale_contrast(gray_image, alpha=1.3)
...
num_clipped_high = (gray_image.astype("float32") * 1.3 > 255).sum()

print("Nonzero bins in original:", (hist_original > 0).sum())
print("Nonzero bins in low-contrast:", (hist_low > 0).sum())
print("Nonzero bins in high-contrast:", (hist_high > 0).sum())

print("Pixels clipped in high-contrast image:", num_clipped_high)

print("First 30 nonzero bins in low-contrast:", [i for i, v in enumerate(hist_low) if v > 0][:30])
print("First 30 nonzero bins in high-contrast:", [i for i, v in enumerate(hist_high) if v > 0][:30])
